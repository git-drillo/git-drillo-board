...
taskController.getCommits = async (req, res, next) => {
  try {
    aToken = req.cookies.accessToken;
  ...
  try {
    ...
    for (let i = 0; i < commits.length; i++) {
      if (commits[i].includes('$$')) {
        ...
        let nickName = newCommits[newCommits.length - 1]
          .substring(0, newCommits[newCommits.length - 1].indexOf('.'))
          .trim();

        const pendingQuery = `
          UPDATE tasks 
          SET ispending='t', commit_msg='${cmtMsg}', commit_url='${commitUrls[i]}' 
          WHERE nick='${nickName}';`;
